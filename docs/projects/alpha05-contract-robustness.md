...
`storage` discriminator, for `exported` fields only (a non-exported field isn't in the
compiled `ledger()` reader, so it can't be read): `Counter` → `BigInteger`; `Cell<T>` → `T`
for Uint/Field/Boolean/`Bytes<N>`/string `Opaque`/`Enum`/`Vector<Uint|Bytes>`/`Maybe<string>`,
delegating to the `MidnightLedger` getters. A field the runtime can't type yet (`Map`/`Set`/
`MerkleTree`, or a Cell of a struct/complex value) is **skipped and listed in the class KDoc**.

Direct `Map`/`Set` accessors were investigated and **deliberately not built**: every Map/Set a
dApp reads is exposed by a typed **view circuit** the contract author wrote (`getProposal`,
`isSigner`, `getApprovalCount`, …), already generated by `read<Name>()`; non-exported Maps are
intentionally private. Direct ADT access would duplicate the view-circuit reads and bypass the
contract's encapsulation. (The runtime *could* do it — the Map/Set getter exposes
`member`/`lookup`/`size` in JS over the existing `__native_contractQuery`, no new FFI — but no
