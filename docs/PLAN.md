...
## Phase 10: MPC-Based Custody & Coordination (Future Research)

**Goal:** Introduce Multi-Party Computation primitives to Kuira as the Midnight ecosystem's MPC / DRek / TEE infrastructure matures. Covers threshold Schnorr shared wallets (M-of-N family / corporate), DRek social recovery, private multi-agent coordination for the Agent Store, and threshold-split agent custody.

**Status:** 🔬 Queued — not a v1.0 or v1.1 item. Activates when (a) Midnight Labs ships DRek / MPC reference infrastructure, (b) mobile-capable threshold Schnorr (FROST family) libraries reach production readiness, or (c) an Agent Store vertical explicitly requires private multi-agent coordination.

### Scope (concept-level)

- **Shared wallets** — M-of-N Schnorr custody for family / corporate use cases; aligns with Charles's AMA ("work: company wallet, M-of-N shared"). Zero single-point-of-loss.
- **Social recovery** — DRek protocol integration: key recovery split across friends, services, or Turnkey-style custodians. Closes the gap flagged in `KUIRA_VISION_V1.md` (seed-phrase MVP → TEE + social recovery migration).
- **Private agent coordination** — the "MPC for Multi-Agent Coordination" research item in `AGENT_STORE_VISION.md`. Use cases: private matching / dark-pool DEX, collaborative compliance, federated agent strategy where no agent sees the full state.
- **Agent custody** — threshold-split agent signing keys between user and policy enforcer for instant revocation without single-key compromise.

### Architectural principle (hard rule)

**Never roll custom MPC.** Always use a generic MPC VM toolchain (MP-SPDZ, or its successor at the time of activation). Custom MPC protocols require cryptography PhDs and a long tail of maintenance; MPC VMs deliver competitive performance and broader threat-model flexibility with far less implementation risk. Same delegation pattern Kuira already applies to single-party crypto via `midnight-ledger` Rust FFI.

Rationale note specific to Midnight: threshold Schnorr (FROST family) is naturally easier to MPC-ify than threshold ECDSA because Schnorr is linear (no `k⁻¹` inversion). The MPC VM case is therefore even stronger on Midnight than in the ECDSA-focused research that originally motivated this principle.

### Entry conditions

Before Phase 10 work begins, all three should hold:

- Midnight Labs DRek / account-abstraction SDK is shipping or imminent.
- A production-grade MPC VM (MP-SPDZ or successor) with a mobile-viable deployment story exists.
- At least one concrete use case from the scope above has a live product need (not just a hypothetical).

### Dependencies

- Phase 7 (Agent Runtime) complete — provides agent identity and execution surface.
- Midnight Labs: DRek reference implementation; potentially threshold primitives in `midnight-ledger`.
- Ecosystem: production threshold Schnorr library usable from Rust / Kotlin on Android.

### Outcomes

- Architectural decision record: which MPC VM, which threat model(s), which verticals.
- Reference integration for one use case end-to-end (likely shared wallets or DRek recovery, whichever ships first upstream).
- Contribution back to ecosystem if we build reusable Kotlin / Rust bindings on top of the chosen MPC VM.
