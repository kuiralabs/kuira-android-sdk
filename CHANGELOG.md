...
- **Dust-backup restore continuity** — the backup preference now travels WITH the wallet instead of dying with the install, and a fresh install prefers restoring the cloud dust checkpoint over a genesis replay. Wallet-level backup prefs (`WalletBackupPrefs`) ride inside the seed-keyed app-state blob via a versioned envelope (legacy blobs decode unchanged; `fetchAppState` still returns exactly the host payload — new `fetchRestoredAppState` exposes prefs + payload together). A host-provided restore gate (`MidnightSdk.Builder.dustRestoreGate`) is awaited once before the first no-checkpoint dust sync: the bundled wallet UI probes the Drive grant silently (a same-app reinstall or new device restores with zero prompts), and when consent is missing shows a blocking in-app "Restore your wallet data?" step — Connect & Restore is the default action, a dismissed system dialog returns to the step for retry, and only an explicit "Skip — re-sync from scratch" (cost stated, never durable) proceeds without the restore. New wallets get a one-time "Keep your wallet synced" setup offer after first bootstrap, so the single Drive grant that makes every future install restore silently happens at onboarding. An explicitly opted-out wallet is never prompted, and the toggle position a fresh install couldn't know is restored. Network-correct by construction: the Drive blob is a per-address bundle and addresses embed the network, pinned by tests (`midnight-sdk` / `wallet-runtime` / `dapp-ui`).
...
- **Balance-time failure no longer wipes the Dust checkpoint** — a failed fee balance (most commonly back-to-back transactions: the previous tx just spent the wallet's dust and its events hadn't reached the indexer yet) went straight to a genesis rebuild, deleting the checkpoint and replaying the full event stream (minutes of dead wallet on a real chain). The recovery now uses the same escalation ladder as the submit-time 170/171 recovery: a fast delta re-sync and retry first — which picks up the just-spent UTXO and its change in seconds — with the genesis rebuild kept only as the last resort for a corrupt checkpoint (`midnight-sdk`).
