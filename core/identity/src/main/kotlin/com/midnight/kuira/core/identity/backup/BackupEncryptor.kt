 ...
 * ```
 * [version: 1 byte] [entropy: 32 bytes] [bip39Seed: 64 bytes] [padding: 399 zeros]
...
object BackupEncryptor {
    ...
    /**
     * Encrypts seed material into a fixed-size blob.
     *
     ...
     * @return 525-byte encrypted blob
     */
    fun encrypt(
        entropy: ByteArray,
        bip39Seed: ByteArray,
        ...
        // Build fixed-size plaintext: version + entropy + seed + padding
        val plaintext = ByteArray(PLAINTEXT_SIZE)
        try {
            plaintext[0] = VERSION
            System.arraycopy(entropy, 0, plaintext, 1, ENTROPY_SIZE)
            System.arraycopy(bip39Seed, 0, plaintext, 1 + ENTROPY_SIZE, SEED_SIZE)
    ...
    fun decrypt(
        ...
        try {
            ...
            val entropy = plaintext.copyOfRange(1, 1 + ENTROPY_SIZE)
            val seed = plaintext.copyOfRange(1 + ENTROPY_SIZE, 1 + ENTROPY_SIZE + SEED_SIZE)

            return DecryptedBackup(entropy = entropy, bip39Seed = seed)
        } finally {
            plaintext.fill(0)
        }
    }
...
/**
 * Decrypted backup contents — the seed material.
