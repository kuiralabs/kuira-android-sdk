...
internal object ContractApiGenerator {
    ...
    private val DECODE_BYTES_MEMBER = MemberName(GENERATED_PACKAGE, DECODE_BYTES)
    ...
    private fun decodeTopLevel(type: JsonObject, returnType: TypeName, supporting: SupportingTypes): CodeBlock {
        ...
        return when (name) {
            "Struct" -> {
                ...
                CodeBlock.of("%M(%T(json))", structDecoderMember(structName), JSON_OBJECT)
            }
            "Bytes" -> {
                supporting.markUsesBytesDecoderIf(true)
                CodeBlock.of("%M(%T(json).nextValue())", DECODE_BYTES_MEMBER, JSON_TOKENER)
            }
    ...
    private fun decodeFieldFromObject(
        ...
        return when (name) {
            "Struct" -> {
                val structName = (resolved["name"] as? JsonString)?.value ?: error("Struct missing name")
                CodeBlock.of("%M(%L.getJSONObject(%S))", structDecoderMember(structName), objVar, fieldName)
            }
            "Bytes" -> {
                supporting.markUsesBytesDecoderIf(true)
                CodeBlock.of("%M(%L.get(%S))", DECODE_BYTES_MEMBER, objVar, fieldName)
            }
