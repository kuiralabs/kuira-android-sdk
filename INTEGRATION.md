...
This guide walks the recipe end-to-end. The two reference apps —
[`examples/midnight-kicks`](examples/midnight-kicks) and
[`examples/bboard`](examples/bboard) — consume the SDK exactly as described
below. If anything here drifts from how they work, prefer the working code.
...
For a real, multi-step contract (commit / reveal / ZK proofs / SD), study
[`examples/midnight-kicks/.../MatchManager.kt`](examples/midnight-kicks/app/src/main/kotlin/com/midnight/kicks/MatchManager.kt)
— it's an idiomatic ~600-line state-machine using every SDK surface you'd
touch.
