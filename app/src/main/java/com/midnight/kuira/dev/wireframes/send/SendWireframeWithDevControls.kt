...
@Composable
fun SendWireframeWithDevControls(
    onBack: () -> Unit = {},
    onOpenWireframeList: () -> Unit = {},
) {
    var screen by remember { mutableStateOf(SendScreen.TOKEN_MODE) }
    var state by remember { mutableStateOf(SendWireframeState.DEFAULT) }
    ...
    if (modalOpen) {
        DevPortalModal(
            ...
            stateControls = {
                DevStateSection(label = "SCREEN") {
                    SendScreen.entries.forEach { s ->
                        DevRadioRow(
                            label = s.name.lowercase().replace('_', ' '),
                            selected = s == screen,
                            onClick = { screen = s; state = SendWireframeState.DEFAULT },
                        )
                    }
                }
                ...
                DevStateSection(label = "STATE") {
                    SendWireframeState.entries.forEach { s ->
                        DevRadioRow(
                            label = s.name.lowercase().replace('_', '-'),
                            selected = s == state,
                            onClick = { state = s },
