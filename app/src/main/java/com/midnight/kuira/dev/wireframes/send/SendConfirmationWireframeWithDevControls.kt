...
@Composable
fun SendConfirmationWireframeWithDevControls(
    onBack: () -> Unit = {},
    onOpenWireframeList: () -> Unit = {},
) {
    var state by remember { mutableStateOf(ConfirmationState.DEFAULT) }
    ...
    if (modalOpen) {
        DevPortalModal(
            title = "Send Confirmation · Dev Controls",
            ...
            stateControls = {
                DevStateSection(label = "STATE") {
                    ConfirmationState.entries.forEach { wireframeState ->
                        DevRadioRow(
