...
@Composable
fun TxHistoryWireframeWithDevControls(
    onBack: () -> Unit = {},
    onOpenWireframeList: () -> Unit = {},
) {
    var screen by remember { mutableStateOf(TxHistoryScreen.LIST) }
    var state by remember { mutableStateOf(TxHistoryState.DEFAULT) }
        ...
        DevPortalModal(
            ...
            stateControls = {
                DevStateSection(label = "SCREEN") {
                    TxHistoryScreen.entries.forEach { s ->
                        DevRadioRow(
                ...
                DevStateSection(label = "STATE") {
                    TxHistoryState.entries.forEach { s ->
                        DevRadioRow(
                            label = s.name.lowercase().replace('_', '-'),
                            selected = s == state,
                            onClick = { state = s },
